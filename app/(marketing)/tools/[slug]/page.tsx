  ...
  return (
    <div className="min-h-screen bg-[#f7f4ee] text-[#191919]">
      ...
      <main className="mx-auto max-w-6xl px-4 py-14 sm:px-6 lg:px-8">
        <section className="rounded-lg border border-black/10 bg-white p-8 shadow-sm">
        ...
        <section className="mt-6 grid gap-4 md:grid-cols-2">
          {page.features_json.map((feature) => (
            <div key={feature} className="flex gap-3 rounded-lg border border-black/10 bg-white p-5 shadow-sm">
        ...
        <section className="mt-6 rounded-lg border border-black/10 bg-white p-6 shadow-sm">
          <p className="text-sm font-black uppercase text-[#0A66C2]">Labeled workflow example</p>
          <h2 className="mt-3 text-2xl font-black text-[#191919]">{page.workflow_example_json.title}</h2>
          <p className="mt-3 text-sm leading-6 text-[#555]">{page.workflow_example_json.body}</p>
        </section>

        <section className="mt-6 space-y-3">
          {page.faq_json.map((faq) => (
            <div key={faq.question} className="rounded-lg border border-black/10 bg-white p-5 shadow-sm">
